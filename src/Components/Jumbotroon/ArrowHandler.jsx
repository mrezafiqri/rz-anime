...
const ArrowHandler = ({ prevSlide, nextSlide }) => {
  return (
    <section className="flex justify-between absolute bottom-[50%] left-0 right-0 px-1 md:px-3 gap-6 transition-all duration-75 invisible group-hover:visible">
      <button
        className="border border-color-white/10 rounded-full p-2 bg-gradient-to-r from-color-white/10 to-color-black/20 backdrop-blur-lg group-hover:border-color-white/60 hover:text-color-primary transition duration-75"
        onClick={prevSlide}
      >
